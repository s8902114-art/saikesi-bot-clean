...
- **監控幣種**: 實際約 **80~110 個**（2026-08-27 實測校正，原本寫「40+」是**硬編碼備援列表**的數字，過期）
  - 底池 = CoinGecko 市值前100 ∩ OKX USDT永續（排除穩定幣）= 實測 **54 幣**
  - ＋ OKX 漲幅前25 / 跌幅前25（`TOP_MOVERS_N=25`，有流動性門檻 `MIN_MOVER_VOLCCY`）
  - ＋ OI 增幅前20（`OI_MOVERS_N=20`，補「主力安靜建倉、價格沒動」的掃描盲區）
  - 扣掉三者重疊 → 實際約 80~110
  - 硬編碼備援 `_SYMBOLS_FALLBACK` = 40 幣，**只有 CoinGecko 抓失敗時才會退回**
  - OKX USDT 永續合約總數約 439，所以掃描覆蓋率約 1/4
- **交易所**: OKX（主）+ BingX（副）
...
| **1H** | — | **★★4J減速跌破空(2026-09-10,全倉上線)**:位階取**2h樞紐高(pv6)**、判定與進場在1H。①價格「爬樓梯」磨到位階(觸及前24根:路徑效率<0.15、漲幅<0.5%、下跌根數≥11)②盤整期間**觸及位階≥2次**(官方「兩次成立」)③進場K量≥2.0x④**ADX(14)≥15**(用戶看輸單圖指出「輸的大多是盤整」,實測ADX<15是唯一負桶)⑤扳機=收盤跌破盤整區低點。停損=盤整區最高點×1.0015、**TP 2R全平、浮盈0.8R移保本**(`fourjd_2r`)。★驗收用**滾動前推**(參數挑選本身也納入檢驗):樣本外n=107 EV+0.554 正期7/8 CI[+0.326,+0.790];逐根重放對拍100%、移植對拍264/264、15m重跑bar內順序僅0.3%不同、成本+0.25%仍正、與吞噬空重疊率1.4%、頻率≈1.3筆/天。★熔斷已寫成代碼:`_fourjd_record_result` 連續吃滿停損8筆(回測最長4)自動停(`FOURJD_MAX_CONSEC_SL`)。★已知弱點:訓練段(23Q4~24Q3)永遠最弱、24Q1一直是負的、2022熊市最肥近期較普通 |
| **1H** | **★BPR失衡區重合(2026-07-08,Bisancos SMC課程挖掘,雙向)**:限主流+4H EMA200 regime對齊,上漲FVG與下跌FVG重疊區回測中軸順延續方向,固定1.5R。WF驗證(major+regime,train/verify同號)EV+0.200/勝60.7%/PF1.44/n=1811/7期正/頻率2.4筆天,跟OI_SQUEEZE重疊率0.9%非重複下注。★觀察倉期:全新策略無live樣本,`BPR_OBSERVATION_SCALE=0.5`半倉,累計30筆人工核對達標(勝率55-65%)再拿掉;連續虧損若超過回測最長記錄8筆應暫停重查(`_check_bpr`) | 同左(雙向) |
