...
### ★V成型 群聚閘 + BTC反彈閘（2026-09-16）
| 功能 | 位置 | 說明 |
|---|---|---|
| 常數/說明 | `VLONG_CROWD_GATE` | V成型=全市場爆倉後集體反彈的事件策略;證據與代價寫在常數旁 |
| 群聚計數 | `def _vlong_crowd_count` / `_VLONG_CROWD` | 過完其他閘的 V 先記名;前4h「別的幣」出V數 ≥ 3×掃描池幣數/163 才進;儀表 `群聚不足` |
| BTC反彈 | `def _btc_rebound_24h` | BTC 15m 收盤/近96根最低 −1 ≥ 3%;抓不到放行;儀表 `BTC未反彈` |
| 對拍 | `_chk_vlong_crowd_port.py` | BTC反彈 600/600、群聚計數 3269/3269 0 不一致 |
