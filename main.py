...
import subprocess
for _pkg in [“requests”, “pandas”, “numpy”, “ccxt”, “flask”, “PyNaCl”]:
try: **import**(_pkg)
...
QQE_RSI = 6; QQE_SF = 5; QQE_FACTOR_P = 3.0; QQE_FACTOR_S = 1.61; QQE_THRESHOLD = 3
ADX_THR = 25; MAX_SL = 0.12; FUNDING_LONG_MAX = 0.0001; FUNDING_SHORT_MIN = -0.0001
CVD_WINDOW = 3; BEAR_MIN_BARS = 20; MAX_CONSEC_LOSS = 3; PAUSE_HOURS = 24

BEST_PARAMS: Dict[str, Dict] = {
“15m_long”:  {“tp1_mult”: 1.725, “tp2_intraday_mult”: 1.8,  “tp2_swing_mult”: 1.8,  “sl_atr_buffer”: 0.01, “structure_lookback”: 28, “exit_mode”: “fixed”},
“15m_short”: {“tp1_mult”: 2.0,   “tp2_intraday_mult”: 3.2,  “tp2_swing_mult”: 3.2,  “sl_atr_buffer”: 0.08, “structure_lookback”: 20, “exit_mode”: “fixed”},
“30m_long”:  {“tp1_mult”: 1.725, “tp2_intraday_mult”: 1.8,  “tp2_swing_mult”: 1.8,  “sl_atr_buffer”: 0.05, “structure_lookback”: 10, “exit_mode”: “fixed”},
“30m_short”: {“tp1_mult”: 2.0,   “tp2_intraday_mult”: 3.2,  “tp2_swing_mult”: 3.2,  “sl_atr_buffer”: 0.01, “structure_lookback”: 10, “exit_mode”: “trailing”},
“1H_long”:   {“tp1_mult”: 1.725, “tp2_intraday_mult”: 2.5,  “tp2_swing_mult”: 2.5,  “sl_atr_buffer”: 0.15, “structure_lookback”: 10, “exit_mode”: “fixed”},
“1H_short”:  {“tp1_mult”: 2.0,   “tp2_intraday_mult”: 4.0,  “tp2_swing_mult”: 4.0,  “sl_atr_buffer”: 0.08, “structure_lookback”: 20, “exit_mode”: “fixed”},
“4H_long”:   {“tp1_mult”: 1.725, “tp2_intraday_mult”: 2.5,  “tp2_swing_mult”: 2.5,  “sl_atr_buffer”: 0.03, “structure_lookback”: 10, “exit_mode”: “trailing”},
“4H_short”:  {“tp1_mult”: 2.0,   “tp2_intraday_mult”: 4.0,  “tp2_swing_mult”: 4.0,  “sl_atr_buffer”: 0.05, “structure_lookback”: 30, “exit_mode”: “fixed”},
“5m_long”:   {“tp1_mult”: 1.725, “tp2_intraday_mult”: 1.8,  “tp2_swing_mult”: 1.8,  “sl_atr_buffer”: 0.08, “structure_lookback”: 20, “exit_mode”: “fixed”},
“5m_short”:  {“tp1_mult”: 2.0,   “tp2_intraday_mult”: 3.2,  “tp2_swing_mult”: 3.2,  “sl_atr_buffer”: 0.08, “structure_lookback”: 20, “exit_mode”: “fixed”},
}
...
for fname in [os.path.join(base_dir, f”best_params_{tf.lower()}*{side}.json”), os.path.join(base_dir, “final_params_all.json”)]:
...
return {“tp1_mult”: 1.7, “tp2_intraday_mult”: 1.8, “tp2_swing_mult”: 2.5, “sl_atr_buffer”: 0.08, “structure_lookback”: 20, “exit_mode”: “fixed”}
...
return
try:
requests.post(f”{DC_BASE}/channels/{DISCORD_CHANNEL_ID}/messages”,
headers=_dc_headers(), json={“content”: text}, timeout=10)
...
try:
r = requests.post(f”{DC_BASE}/channels/{DISCORD_CHANNEL_ID}/messages”,
headers=_dc_headers(), json=payload, timeout=10)
...
return
try:
requests.patch(f”{DC_BASE}/channels/{DISCORD_CHANNEL_ID}/messages/{message_id}”,
headers=_dc_headers(), json={“content”: content, “components”: []}, timeout=10)
except Exception as e:
print(f”  [DC edit] {e}”)

def dc_signal(sig: Dict, symbol: str, tf: str, cvd_active: bool) -> str:
side_emoji = “GREEN” if sig[“side”] == “long” else “RED”
dir_s      = “做多” if sig[“side”] == “long” else “做空”
swing_tag  = “波段” if sig[“is_swing”] else “日內”
cvd_tag    = “CVD+” if cvd_active else “CVD-”
...
embed = {
“title”: f”{coin} [{tf} {dir_s}] {swing_tag} {cvd_tag}”,
“color”: color,
“fields”: [
...
coin = symbol.split(”/”)[0]
dc(f”{coin} [{tf} {side.upper()}] 出場\n{msg}”)

def dc_pause(symbol: str, tf: str, side: str, resume: datetime):
coin = symbol.split(”/”)[0]
dc(f”暫停 {coin} [{tf} {side.upper()}] 連虧{MAX_CONSEC_LOSS}單\n”
...
ts = _okx_ts()
return {
“OK-ACCESS-KEY”: OKX_API_KEY, “OK-ACCESS-SIGN”: _okx_sign(ts, method, path, body),
...
rows = sorted(_okx_pub(”/api/v5/market/candles”, {“instId”: inst_id, “bar”: bar, “limit”: min(limit, 300)}), key=lambda x: int(x[0]))
if not rows:
return pd.DataFrame()
...
data = _cona_get(“history”, {“symbols”: cona_sym, “interval”: cona_iv, “from”: from_ms // 1000, “to”: to_ms // 1000, “convert_to_usd”: “false”})
rows = []
for item in data:
...
if not COINALYZE_API_KEY:
return pd.Series(dtype=float)
data = _cona_get(“open-interest-history”, {“symbols”: cona_sym, “interval”: cona_iv, “from”: from_ms // 1000, “to”: to_ms // 1000})
rows = []
for item in data:
if not isinstance(item, dict):
continue
...
def _make_okx_ex():
ex = ccxt.okx({“apiKey”: OKX_API_KEY, “secret”: OKX_SECRET, “password”: OKX_PASSPHRASE, “options”: {“defaultType”: “swap”}})
if OKX_DEMO:
...
def place_okx_order(symbol: str, direction: str, entry: float, sl: float, tp1: float, tp2: float):
global _LIVE_MODE, MAX_LEVERAGE
if not _LIVE_MODE:
dc(“Paper 模式，未下單。輸入 !setlive 切換實盤”)
return
try:
ex = _make_okx_ex()
ex.load_markets()
...
dc(f”可用：{avail:.1f}U  保證金：{margin:.1f}U  倉位：{pos_val:.1f}U  槓桿：{sug_lev}x”)
is_l     = direction == “long”
es, xs   = (“buy”, “sell”) if is_l else (“sell”, “buy”)
pos_side = “long” if is_l else “short”
# 進場（先嘗試雙向持倉模式，失敗則 fallback）
try:
eo = ex.create_market_order(symbol, es, amt, params={“posSide”: pos_side, “tdMode”: “isolated”})
except Exception:
eo = ex.create_market_order(symbol, es, amt)
res = [f”進場 {symbol} {‘做多’ if is_l else ‘做空’} {sug_lev}x\n均價:{eo.get(‘average’) or price}”]
# 止損
try:
o = ex.create_order(symbol, “stop_market”, xs, amt, None, {“stopPrice”: sl, “reduceOnly”: True, “posSide”: pos_side})
res.append(f”SL {sl} ID:{o.get(‘id’)}”)
except Exception:
try:
o = ex.create_order(symbol, “stop_market”, xs, amt, None, {“stopPrice”: sl, “reduceOnly”: True})
res.append(f”SL {sl} ID:{o.get(‘id’)}”)
except Exception as e2:
res.append(f”SL失敗:{e2}”)
# 止盈
...
if order:
Thread(target=place_okx_order, args=(order[“symbol”], order[“direction”], order.get(“entry”, 0), order[“sl”], order[“tp1”], order[“tp2”]), daemon=True).start()
msg_id = order.get(“msg_id”)
if msg_id:
Thread(target=dc_edit, args=(msg_id, “已確認下單，執行中…”), daemon=True).start()
return jsonify({“type”: 6})
else:
return jsonify({“type”: 4, “data”: {“content”: “訊號已過期”, “flags”: 64}})
...
val = float(parts[1])
if val <= 0 or val > 100:
dc(“請輸入 0.1~100”); return
MARGIN_PCT = val
dc(f”每倉保證金比例已設為 {val}%”)
except ValueError:
dc(“格式錯誤，例：!setrisk 5”)
else:
dc(f”目前每倉保證金：可用餘額 x {MARGIN_PCT}%”)
...
dc(“格式錯誤，例：!setmaxlev 50”)
else:
dc(f”目前最高槓桿上限：{MAX_LEVERAGE}x”)
elif text.startswith(”!pause”):
_PAUSED = True
dc(“訊號已暫停。輸入 !resume 恢復”)
elif text.startswith(”!resume”):
_PAUSED = False
dc(“訊號已恢復”)
elif text.startswith(”!setlive”):
if not OKX_API_KEY:
dc(“尚未設定 OKX_API_KEY”); return
_LIVE_MODE = True
dc(“已切換為實盤模式”)
...
for k in AUTO_TRADE:
AUTO_TRADE[k] = onoff
dc(f”全部時框自動下單已{‘開啟’ if onoff else ‘關閉’}”)
elif tf_arg in AUTO_TRADE:
AUTO_TRADE[tf_arg] = onoff
dc(f”{tf_arg} 自動下單已{‘開啟’ if onoff else ‘關閉’}”)
else:
dc(“時框請輸入 15m / 30m / 1H / 4H / ALL”)
else:
...
margin_str = f”{avail * MARGIN_PCT / 100:.1f} U（可用x{MARGIN_PCT}%）”
...
paused_str = “暫停中” if _PAUSED else “運行中”
auto_str = “ | “.join(f”{k}:{‘on’ if v else ‘off’}” for k, v in AUTO_TRADE.items())
dc(f”目前狀態\n訊號：{paused_str}\n模式：{mode}\n餘額：{bal_str}\n每倉保證金：{margin_str}\n最高槓桿：{MAX_LEVERAGE}x\n未平倉（Paper）：{open_cnt} 筆\n已運行：{h}h {m}m\n監控：{len(SYMBOLS)} 幣 | {len(TIMEFRAMES)} 時框\n自動下單：{auto_str}\nCVD：{‘Coinalyze’ if COINALYZE_API_KEY else ‘無’}”)
elif text.startswith(”!help”):
dc(“指令列表\n!pause / !resume — 暫停/恢復訊號\n!setrisk [%] — 每倉保證金比例\n!setmaxlev [x] — 最高槓桿上限\n!setlive / !setpaper — 切換實盤/模擬\n!auto 1H on/off — 自動下單開關\n!auto all on/off — 全部時框\n!autostatus — 自動下單狀態\n!status — 目前狀態\n!help — 此說明”)
...
try:
r = requests.get(f”{DC_BASE}/channels/{DISCORD_CHANNEL_ID}/messages”, headers=_dc_headers(), params={“limit”: 1}, timeout=15)
msgs = r.json()
...
while True:
try:
r = requests.get(f”{DC_BASE}/channels/{DISCORD_CHANNEL_ID}/messages”, headers=_dc_headers(), params={“after”: _dc_last_msg_id, “limit”: 10}, timeout=15)
msgs = r.json()
if isinstance(msgs, list) and msgs:
seen = set()
...
next_closes = {tf: (int(now / BAR_SECONDS[tf]) + 1) * BAR_SECONDS[tf] + 5 for tf in TIMEFRAMES}
...
atr_ = *ws(tr_a, period); pdi = *ws(pdm, period); mdi = *ws(mdm, period)
...
def calc_qqe(close: pd.Series, rsi_period: int, sf: int, factor: float, threshold: float = 3.0):
delta = close.diff()
...
dar    = (atrRsi.ewm(span=rsi_period, adjust=False).mean().ewm(span=rsi_period, adjust=False).mean() * factor)
...
df[“qtr”]   = np.where((tr1 == 1) & (tr2 == 1), 1, np.where((tr1 == -1) & (tr2 == -1), -1, 0)).astype(np.int8)
return df

def is_bear_market(df_1h: pd.DataFrame, min_bars: int = BEAR_MIN_BARS) -> bool:
e144 = df_1h[“e144”].values; e576 = df_1h[“e576”].values; count = 0
...
pts = [w[j] for j in range(n, len(w) - n) if all(w[j] < w[j - k] for k in range(1, n + 1)) and all(w[j] < w[j + k] for k in range(1, n + 1))]
...
def check_signal(df: pd.DataFrame, params: Dict, side: str, funding_now: float = float(“nan”), bear_market: bool = False) -> Optional[Dict]:
i = len(df) - 1
if i < 10: return None
...
if not (cl[i] < e12[i]):    return None
base = *piv_high(hi, i, lb)
if math.isnan(base):        return None
sl   = base + atr*[i] * buf
risk = sl - cl[i]
cvd_result, cvd_active = check_cvd(df, side)
if cvd_result is False: return None
entry = cl[i]
...
return {
“side”: side, “time”: df.index[i].isoformat(), “bar_ts”: df.index[i].isoformat(),
...
“rr1”: round(tp1_mult, 3), “rr2”: round(tp2_mult, 3),
“exit_mode”: exit_mode, “is_swing”: is_swing,
“atr”: round(float(atr_[i]), 6), “cvd_active”: cvd_active,
}

class PaperPosition:
def **init**(self, signal: Dict, symbol: str, tf: str):
self.symbol = symbol; self.tf = tf; self.side = signal[“side”]
self.entry = signal[“entry”]; self.sl = signal[“sl”]
self.tp1 = signal[“tp1”]; self.tp2 = signal[“tp2”]
self.exit_mode = signal[“exit_mode”]; self.tp1_hit = False; self.open = True
    ...
    if not self.tp1_hit:
        ...
        if self.exit_mode == "fixed":
            ...
            if rev_q or rev_e12 or rev_tr:
                reason = "QQE" if rev_q else "EMA12" if rev_e12 else "EMA144/576"
                self.open = False; return f"Trail exit ({reason}) @ {c:.4f}"
    return None
```

class TradingBotV3:
def **init**(self, tf_plan: List[Tuple], sides: List[str], live: bool = False, base_dir: str = “.”):
self.tf_plan = tf_plan; self.sides = sides; self.live = live; self.base_dir = base_dir
self.params: Dict[str, Dict] = {}
for tf, *, tf_sides in tf_plan:
for s in tf_sides:
if s in sides:
self.params[f”{tf}*{s}”] = get_params(tf, s, base_dir)
...
self.*bear_cache: Dict[str, bool] = {}
for tf, *, tf_sides in tf_plan:
for inst_id in SYMBOLS:
for s in tf_sides:
if s in sides:
key = f”{inst_id}*{tf}*{s}”
self.positions[key] = None; self.consec_loss[key] = 0
self.pause_until[key] = None; self.last_signal_time[key] = 0.0
...
def _record_loss(self, key, tf):
    ...
    if self.consec_loss[key] >= MAX_CONSEC_LOSS:
        ...
        self.pause_until[key] = resume; self.consec_loss[key] = 0
        parts = key.split("_"); sym = parts[0] + "-USDT-SWAP"; label = SYMBOLS.get(sym, sym)
        dc_pause(label, tf, parts[-1], resume)

def _record_win(self, key): self.consec_loss[key] = 0
...
def _update_bear_cache(self):
    for inst_id, label in SYMBOLS.items():
        ...
        print(f"  {label.split('/')[0]}: {'BEAR' if self._bear_cache[inst_id] else 'BULL'}")
        sleep(0.2)

def _scan(self, inst_id, label, tf, side, df, funding_now):
    key = self._key(inst_id, tf, side)
    p   = self.params.get(f"{tf}_{side}", {})
    if self._is_paused(key): return
    pos = self.positions[key]
    if pos and pos.open:
        msg = pos.update(df)
        if msg:
            print(f"  EXIT {label.split('/')[0]} [{tf} {side}]: {msg}")
            dc_exit(label, tf, side, msg)
            if "SL" in msg or "Breakeven" in msg: self._record_loss(key, tf)
            else: self._record_win(key)
            self.positions[key] = None; pos = None
    if pos and pos.open: return
    bear = self._bear_cache.get(inst_id, False)
    sig  = check_signal(df, p, side, funding_now=funding_now, bear_market=bear)
    if not sig: return
    now_ts = time.time()
    if now_ts - self.last_signal_time.get(key, 0) < SIGNAL_COOLDOWN: return
    self.last_signal_time[key] = now_ts
    if _PAUSED:
        print(f"  [PAUSED] {label} {tf} {sig['side']}"); return
    coin = label.split("/")[0]
    print(f"  {'LONG' if sig['side']=='long' else 'SHORT'} {coin} [{tf}]  Entry:{sig['entry']}  SL:{sig['sl']}")
...
def _scan_symbol_tf(self, inst_id, label, tf, tf_sides, all_sides):
    ...
    if df_raw.empty or len(df_raw) < 200: return
    df = precompute_base(df_raw); df = add_swing(df); df = add_dual_qqe(df)
...
def scan_once(self, due_tfs: Optional[List[str]] = None):
    ...
    for tf, _, tf_sides in self.tf_plan:
        if tf not in tfs_to_scan: continue
        print(f"\n-- 掃描 {tf} --")
        for inst_id, label in SYMBOLS.items():
...
def run(self):
    global _bot_ref
    _bot_ref = self
    print(f"賽克斯機器人 v4 | {len(SYMBOLS)} 幣 | {TIMEFRAMES}")
    dc(f"賽克斯機器人 v4 已啟動\n監控：{len(SYMBOLS)} 個幣\n時框：{' / '.join(TIMEFRAMES)}\n模式：{'實盤' if _LIVE_MODE else 'Paper（!setlive 切換）'}\n輸入 !help 查看指令")
    while True:
        try:
            ...
            print("\nStopped."); dc("賽克斯機器人已停止。"); break
        except Exception as e:
            print(f"\n  [ERR] {e}"); dc(f"錯誤：{e}"); sleep(60)
```
...
if args.tf == “all”: tf_plan = DEFAULT_TF_PLAN
elif args.tf in ALL_TF_POLL: tf_plan = [(args.tf, ALL_TF_POLL[args.tf], sides)]
else: tf_plan = DEFAULT_TF_PLAN
if args.live and not OKX_API_KEY: args.live = False
Thread(target=run_web, daemon=True).start()
Thread(target=poll_dc_commands, daemon=True).start()
bot = TradingBotV3(tf_plan=tf_plan, sides=sides, live=args.live, base_dir=args.dir)
if args.once: bot.scan_once()
else: bot.run()
